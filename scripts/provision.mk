...
pro/init:  ##@provision Run ansible to init 
	(cd playbook && ansible-playbook -v init.yml --private-key=$(DEV_KEY) --extra-vars "my_user=$(VM_USER)" $(PRO_PARAMS))

.PHONY: pro/desktop
pro/desktop:  ##@provision Run ansible to setup desktop
	(cd playbook && ansible-playbook -v desktop.yml --private-key=$(DEV_KEY) --extra-vars "my_user=$(VM_USER)" $(PRO_PARAMS))

.PHONY: pro/dev
pro/dev:  ##@provision Run ansible to setup machine with dev tools
	(cd playbook && ansible-playbook -v dev.yml --private-key=$(DEV_KEY) --extra-vars "my_user=$(VM_USER)" $(PRO_PARAMS))
...
pro/snaps:  ##@provision Run ansible to setup machine with snaps
	(cd playbook && ansible-playbook -v snaps.yml --private-key=$(DEV_KEY) --extra-vars "my_user=$(VM_USER)" $(PRO_PARAMS))

.PHONY: pro/snaps-alternative
pro/without-snaps:  ##@provision Run ansible to setup machine without snaps
	(cd playbook && ansible-playbook -v snaps-alternative.yml --private-key=$(DEV_KEY) --extra-vars "my_user=$(VM_USER)" $(PRO_PARAMS))

.PHONY: pro/misc
pro/misc:  ##@provision Run ansible to setup office and vm tools 
	(cd playbook && ansible-playbook -v misc.yml --private-key=$(DEV_KEY) --extra-vars "my_user=$(VM_USER)" $(PRO_PARAMS))

.PHONY: pro/security
pro/security:  ##@provision Run ansible to setup machine's security
	(cd playbook && ansible-playbook -v security.yml --private-key=$(DEV_KEY) --extra-vars "my_user=$(VM_USER)" $(PRO_PARAMS))